...
# 同步当日的k线数据 每日执行
# 同步当日个股k线数据
BaoStockHelper().conn().conn_bs().sync_all_stock_k_day_to_temp().cp_from_temp().clear_k_data_temp().dis_conn().dis_conn_bs()
# 同步当日的指数k线数据
...
(BaoStockIndexHelper().conn().conn_bs().clear_temp_index_data().sync_index_k_day_to_temp(
 ...
 .cp_index_k_day_from_temp(index_code_list=index_code_list).clear_temp_index_data().dis_conn().dis_conn_bs())
